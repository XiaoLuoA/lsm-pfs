...
int test_write(int round, int r, int f) {
...
int test_write_nocache(int round, int r, int f) {
    for (int i = 0; i < round; i++) {
		int lba;
		if (f)
			lba = rand() % (1 << r);
...
int read_lba(int lba, char *buf){
	...
	{
		data_read(lba, buf);
		char *buf3 = put_to_cache(lba, buf, data_cache);
		if (buf3 != nullptr)
			free(buf3);
    }
}

int write_lba(int lba, char *buf){
	int pop_lba;
	char *buf2 = put_to_cache(lba, buf, pop_lba, data_cache);
	if (buf2 != nullptr) {
		data_write(pop_lba, buf, superBlock.data_size);
...
int test_read_nocache(int round, int r, int f) {
	int *a = new int[round];
	for (int i = 0; i < round; i++)
		a[i]=rand() % (1 << r);
	random_shuffle(a, a+round);	
	for (int i = 0; i < round; i++) {
		int lba;
		if (f)
			lba = a[i];
...
int test_read(bool seq, int bs) {
	int blocks = (1LL << 34) / BLOCK_SIZE;
	int lba;
	bs *= 1024;
	for (int i = 0; i < (1LL << 34) / bs; i++) {
		int random_start = rand() % blocks;
		for (int j = 0; j < bs / BLOCK_SIZE; j++){
			if (seq)
				lba = i * (bs / BLOCK_SIZE) + j;
			else
				lba = (random_start + j) % blocks;
			//printf("%d\n",lba);
			char buf[BLOCK_SIZE];
			read_lba(lba, buf);
		}
...
int test_read_ext4(bool seq, int bs) {
	int blocks = (1LL << 34) / BLOCK_SIZE;
	int lba;
	bs *= 1024;
	for (int i = 0; i < (1LL << 34) / bs; i++) {
...
int test_write_ext4(bool seq, int bs) {
	int blocks = (1LL << 34) / BLOCK_SIZE;
	int lba;
	bs *= 1024;
	for (int i = 0; i < (1LL << 34) / bs; i++) {
